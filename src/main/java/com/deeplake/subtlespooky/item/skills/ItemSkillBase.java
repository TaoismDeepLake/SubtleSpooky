package com.deeplake.subtlespooky.item.skills;

import java.util.List;

import com.deeplake.subtlespooky.IdlFramework;
import com.deeplake.subtlespooky.init.ModCreativeTab;
import com.deeplake.subtlespooky.item.ItemBase;
import com.deeplake.subtlespooky.util.CommonDef;
import com.deeplake.subtlespooky.util.CommonFunctions;
import com.deeplake.subtlespooky.util.IDLSkillNBT;
import com.deeplake.subtlespooky.util.MessageDef;
...
public class ItemSkillBase extends ItemBase {
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "subtlespooky.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("subtlespooky.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("subtlespooky.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("subtlespooky.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("subtlespooky.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("subtlespooky.skill.shared.lv_max") : "";

        return I18n.format("subtlespooky.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("subtlespooky.skill.shared.duration_desc", val);
    }
