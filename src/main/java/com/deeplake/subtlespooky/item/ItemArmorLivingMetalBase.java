		...
		if (!worldIn.isRemote)
        {
            if (((float)stack.getItemDamage() / stack.getMaxDamage()) >= 0.9f)
            {
                if (worldIn.getWorldTime() % CommonDef.TICK_PER_SECOND == 0 && !IDLNBTUtil.GetBoolean(stack, IDLNBTDef.BIOMETAL_WARNED, false)) {
                    ...
                    CommonFunctions.SendMsgToPlayer((EntityPlayerMP) player, "subtlespooky.msg.low_dura", stack.getDisplayName());
                }
            }
            else {
...
//
//        //tooltip.add(I18n.format("subtlespooky.gua_enhance_total.desc", IDLSkillNBT.GetGuaEnhanceTotal(stack)));
